shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    if ("no items returned" %in% items | is.null(items)) {
      shiny::showNotification(
        "no items returned from S3 query",
        type = "error",
        duration = 5)
  ...
  report_active_df <- reactive({
    ...
    # don't update options if selected layer has no records
    req(nrow(report_active_df) > 0, "sf" %in% class(report_active_df))
  ...
  observeEvent(input$generate_report, {
    ...
    if (input$make_chart == TRUE) {
      chart_df <- report_active_df() %>%
        dplyr::select(tidyselect::all_of(c(input$report_vars, input$report_group_vars)))
