shinyServer(function(input, output, session) {
  ...
  # get list of GeoPackages in s3 bucket that a user can view
  observe({
    req(data_file$token)
    req(aws_bucket)
  ...
  output$map <- leaflet::renderLeaflet({
    base_map <- leaflet::leaflet() %>%
      leaflet::addTiles() %>%
      leaflet::addTiles(
        urlTemplate = "https://storage.googleapis.com/vavau-tax-allotment/vavau-tiles/{z}/{x}/{y}.png",
        group = "Vava'u Tax Allotments",
      ...
      leaflet::addLayersControl(
        baseGroups = c("OSM (default)", "Vava'u Tax Allotments", "ESRI Satellite"),
  ...
  table_active_df <- reactive({
    ...
    table_active_df <- try(read_tables(dt, input$table_active_layer))
  ...
  view_df <- reactive({
    ...
    if (!is.null(input$table_vars) & length(input$table_vars) > 0) {
      view_df <- try(view_df %>%
        dplyr::select(all_of(input$table_vars)))
    }
  ...
  output$download_map_data <- downloadHandler(
    filename = function() {
      req(nrow(data_file$data_file) >= 1)

      paste("map_data_", dt(), ".zip", sep = "")
    },
    content = function(file) {
      req(nrow(data_file$data_file) >= 1)

      dpath <- data_file$data_file
      # row 1 should be the path to the geopackage
  ...
  # get user selected Google Cloud Storage as layer
  ...
  report_active_df <- reactive({
    ...
    report_active_df <- try(read_tables(df, input$report_active_layer))
  ...
  observeEvent(input$preview_map, {
    ...
    map_df <- report_active_df() %>%
      dplyr::select(all_of(input$report_vars))
  ...
  observeEvent(input$generate_report, {
    ...
    data_file$report_raw_table <- map_df %>%
      sf::st_drop_geometry() %>%
      as.data.frame()
